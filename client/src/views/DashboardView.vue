...
<template>
  <AppLayout>
    ...
    <p v-if="error" class="mb-4 rounded bg-red-50 px-4 py-3 text-sm text-red-600">
      {{ error }}
    </p>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div class="flex items-center justify-between rounded-lg bg-white p-6 shadow">
        <div>
          <p class="text-sm font-medium text-gray-500">Total Folder</p>
          <div class="mt-1">
            <p v-if="loading" class="h-8 w-16 animate-pulse rounded bg-gray-200"></p>
            <p v-else class="text-3xl font-bold text-gray-800">{{ stats?.total_folders ?? 0 }}</p>
          </div>
        </div>
      ...
      <div class="flex items-center justify-between rounded-lg bg-white p-6 shadow">
        <div>
          ...
          <div class="mt-1">
            <p v-if="loading" class="h-8 w-16 animate-pulse rounded bg-gray-200"></p>
            <p v-else class="text-3xl font-bold text-gray-800">{{ stats?.total_files ?? 0 }}</p>
          </div>
        </div>
      ...
      <div class="flex items-center justify-between rounded-lg bg-white p-6 shadow">
        <div>
          ...
          <div class="mt-1">
            <p v-if="loading" class="h-8 w-16 animate-pulse rounded bg-gray-200"></p>
            <p v-else class="text-3xl font-bold text-gray-800">{{ stats?.total_departments ?? 0 }}</p>
          </div>
        </div>
    ...
    <div class="mt-8 overflow-hidden rounded-lg bg-white shadow">
      ...
      <p v-if="loading" class="px-6 py-10 text-center text-sm text-gray-500">Loading...</p>

      <p v-else-if="stats && stats.recent_files.length === 0" class="px-6 py-10 text-center text-sm text-gray-500">

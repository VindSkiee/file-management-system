...
watch(showTrashed, () => {
  getDepartments(showTrashed.value, 1)
})

function goTo(nextPage: number): void {
  if (nextPage < 1 || nextPage > lastPage.value) return
  getDepartments(showTrashed.value, nextPage)
}

onMounted(() => {
  getDepartments(false, 1)
})
...
async function handleSubmit(): Promise<void> {
  ...
  try {
    ...
    if (ok) {
      showModal.value = false
      await getDepartments(showTrashed.value, page.value)
    } else {
...
async function handleDelete(department: Department): Promise<void> {
  ...
  if (ok) await getDepartments(showTrashed.value, page.value)
}
